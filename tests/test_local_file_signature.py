...
def test_nonexistent_relative_path_returns_none(images_root: Path) -> None:
    """相对路径在图片目录下无候选文件时返回 None。"""
    del images_root
    assert ImagePreparer._local_candidate("nope.png") is None


def test_absolute_path_outside_scope_returns_none(images_root: Path) -> None:
    """绝对路径在读权限（工作区 ∪ 图片目录）之外时返回 None，避免越界文件成为存在性 oracle。"""
    del images_root
...
def test_directory_named_as_image_returns_none(images_root: Path) -> None:
    """图片目录内与图片同名的目录不可作为候选。"""
    (images_root / "photo.png").mkdir()
    assert ImagePreparer._local_candidate("photo.png") is None


def test_oversized_candidate_returns_none(
    images_root: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """超过大小上限的候选返回 None。
    ...
    (images_root / "photo.png").write_bytes(b"\x00" * (64 * 1024 + 1))
    assert ImagePreparer._local_candidate("photo.png") is None


def test_candidate_delegates_to_shared_candidate_resolution(images_root: Path) -> None:
...
def test_final_component_symlink_follows_target_within_scope(images_root: Path) -> None:
    """读权限内符号链接按 resolve 跟随语义取目标文件的候选，与读取路径锁定同一文件。"""
    target = images_root / "real.png"
    target.write_bytes(_PNG_BYTES)
    link = images_root / "link.png"
...
def test_final_component_symlink_escaping_scope_returns_none(images_root: Path) -> None:
