...
``response_reports_failure`` 被 common.py 与 results.py 共用，驱动 isError、自动保存
...
from seedream_mcp.utils.core.errors import response_reports_failure
...
def test_status_failed_marks_generation_failed_even_when_success_true() -> None:
    ...
    assert response_reports_failure({"success": True, "status": "failed"}) is True


def test_status_completed_with_success_true_not_failed() -> None:
    """success=True 且 status 非 failed 视为成功。"""
    assert response_reports_failure({"success": True, "status": "completed"}) is False


def test_success_false_marks_failed_regardless_of_status() -> None:
    """success=False 视为失败，无论 status 是否存在或与之冲突。"""
    assert response_reports_failure({"success": False}) is True
    assert response_reports_failure({"success": False, "status": "completed"}) is True


def test_success_true_without_status_not_failed() -> None:
    """success=True 且 status 缺省视为成功，不因 status 字段缺失而误判失败。"""
    assert response_reports_failure({"success": True}) is False
