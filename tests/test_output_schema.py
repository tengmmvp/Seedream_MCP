"""工具 outputSchema（结构化输出声明）测试。

验证所有工具都声明了 outputSchema（MCP 2025-06-18 规范），供客户端程序化解析与校验。
outputSchema 由 FastMCP 依据工具返回类型注解（GenerationStructuredOutput /
BrowseImagesStructuredOutput）自动生成；运行时工具仍返回手动构造的 CallToolResult，
...
async def test_all_tools_declare_output_schema() -> None:
    ...
    for tool in tools:
        assert tool.outputSchema is not None, f"{tool.name} 缺少 outputSchema"
