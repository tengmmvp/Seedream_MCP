...
def test_call_save_path_scope_grants_call_level_read_access(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """save_path 写入目录置位期间并入读权限尾部，退出作用域即收回。

    调用内读写资格不沉淀：界外目录仅在本调用可读，下一次访问回到原读权限。
    """
    ws = tmp_path / "ws"
    ws.mkdir()
    monkeypatch.setenv("SEEDREAM_WORKSPACE_ROOT", str(ws))
    monkeypatch.setattr(io_path_module, "_env_value_providers", {})
    outside = (tmp_path / "exports").resolve()

    before = io_path_module.get_read_scope()
    assert outside not in before

    with io_path_module.call_save_path_scope(outside):
        assert io_path_module.get_read_scope() == [*before, outside]

    assert io_path_module.get_read_scope() == before


def test_call_save_path_scope_none_and_duplicate_pass_through(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """未提供 save_path 时透传不改读权限；与既有成员相同的目录不重复追加。"""
    ws = tmp_path / "ws"
    ws.mkdir()
    monkeypatch.setenv("SEEDREAM_WORKSPACE_ROOT", str(ws))
    monkeypatch.setattr(io_path_module, "_env_value_providers", {})

    baseline = io_path_module.get_read_scope()
    with io_path_module.call_save_path_scope(None):
        assert io_path_module.get_read_scope() == baseline

    with io_path_module.call_save_path_scope(baseline[-1]):
        assert io_path_module.get_read_scope() == baseline
