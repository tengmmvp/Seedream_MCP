...
- 自动保存默认开启：生成结果自动下载到图片保存目录（`SEEDREAM_AUTO_SAVE_BASE_DIR` 配置值或 `<工作位置>/.seedream/images`，工作位置取 MCP Roots 首项、`SEEDREAM_WORKSPACE_ROOT`、进程启动目录或用户主目录）的 `<日期>/<工具名>/` 子目录，文件名含 prompt 词干便于检索
