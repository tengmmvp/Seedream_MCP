...
- 自动保存默认开启：生成结果自动下载到图片目录（`<数据根目录>/.seedream/images`，数据根目录取 `SEEDREAM_DATA_ROOT` 配置值或工作根目录，工作根目录取 MCP Roots 首项、`SEEDREAM_WORKSPACE_ROOT`、进程启动目录或用户主目录）的 `<日期>/<工具名>/` 子目录，文件名含 prompt 词干便于检索
