...
def build_web_request_context() -> _WebRequestContextStub | None:
    ...
    返回 None 表示资源不在堂（uvicorn serve 之外的调用窗口），调用方回退
    ctx=None 路径由流水线按请求新建 client。读取的是模块属性而非 import 期
    绑定，跟随 resources 单例的重建与退役。
    """
